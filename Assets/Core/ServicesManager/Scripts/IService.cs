...
namespace Core.ServicesManager
{
	...
	/// <remarks>Locator creates each service, initializes declared dependencies first, then calls <see cref="Reset"/> in reverse order during teardown. Implementations own their runtime state and must cancel or dispose owned asynchronous work in <see cref="Reset"/>.</remarks>
	public interface IService
	{
		/// <summary>Initializes runtime state after declared dependencies are available.</summary>
		/// <returns><see langword="true"/> when initialization succeeded; <see langword="false"/> stops locator initialization.</returns>
